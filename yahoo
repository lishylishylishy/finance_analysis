...
# 1. 用户配置区 (请在此填写你的信息)
# ==========================================
DEEPSEEK_API_KEY = "AIzaSyAFOoSP9Vk6vZQUin1wilupCYkYsqkrFJ0"  #使用模型Gemini 2.5 Flash TTS
...
def get_ai_analysis(corr_df):
    ...
    client = OpenAI(
        ...
        base_url="https://generativelanguage.googleapis.com/v1beta/openai/" # 如果换 Claude，需修改此 URL 和模型名
    )
...
if st.button("🚀 执行一键同步与 AI 诊断"):
    if not DEEPSEEK_API_KEY.startswith("sk-"):
        st.error("错误：请先在代码开头的配置区填写有效的 DeepSeek API Key！")
    else:
